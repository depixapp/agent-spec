...
const checks = [
  ...
  // (above) and the operator-run npx deployment's 62. The local surplus splits
  // into the 29 `wallet_*` tools and the 7 agent-local tools, both existing
  ...
  ["agent.json mcp.wallet.tool_count_local", pin.mcp_tool_count_local, agent.mcp?.wallet?.tool_count_local],
];
